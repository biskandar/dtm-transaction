...
public class MonthlyClientTrackInit implements Job {
  ...
  static final DLogContext lctx = new SimpleContext( "MonthlyClientTrackInit" );
  ...
  private Map generateCandidateClientIds( int clientid ) {
    ...
    // compose sql
    String sqlSelect = "SELECT c.client_id , cl.monthly_traffic_quota ";
    String sqlFrom = "FROM `client` c ";
    String sqlJoin = "INNER JOIN client_level cl ON cl.id = c.client_level_id ";
    String sqlWhere = "WHERE ( cl.monthly_traffic_quota > 0 ) ";
    if ( clientid > 0 ) {
    ...
    while ( iter.hasNext() ) {
      ...
      stemp = (String) qi.get( 1 ); // monthly_traffic_quota
      if ( ( stemp != null ) && ( !stemp.equals( "" ) ) ) {
        try {
  ...
  private boolean processClient( Integer clientId , Double initUnit ) {
    ...
    String profileId = AccountProfile.MONTHLY_CLIENT_TRACK;
    ...
    if ( billingResult.getPaymentResult() == BillingStatus.PAYMENT_RESULT_SUCCEED ) {
      DLog.debug( lctx , headerLog + "Updated monthly client track balance = "
